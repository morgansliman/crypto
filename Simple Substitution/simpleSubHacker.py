...
import os, re, copy, pprint, pyperclip, detectEnglish, simpleSubCipher, makeWordPatterns
...
def removeSolvedLettersFromMapping(letterMapping, message):
            ...
            for s in solvedLetters:
                if len(letterMapping[cipherletter]) != 1 and s in letterMapping[cipherletter]:
                    ...
                    if len(letterMapping[cipherletter]) == 1:
                        ...
                        loopAgain = True
    return findMissingWords(letterMapping, message)
...
def hackSimpleSub(message):
    ...
    return removeSolvedLettersFromMapping(intersectedMap, message)
...
def findMissingWords(letterMap, ciphertext):
    ...
    ciphertextList = nonLettersOrSpacePattern.sub('', ciphertext.upper()).split()
        ...
        for i in range(len(plaintextList)):
            ...
            if plainword not in dict and '_' in plainword:
                ...
                if numWords == 1 and possibleLetters[0][0] not in solvedLetter:
                    solvedLetter[possibleLetters[0][0]] = [possibleLetters[0][1]]
